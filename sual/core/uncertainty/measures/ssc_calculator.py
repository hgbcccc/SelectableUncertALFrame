...
class SSCCalculator:
    def __init__(self, data_cfg: Dict, weights: Optional[List[float]] = None):
        ...
        # 设置权重参数
        self.weights = weights if weights is not None else [2, 1, 2, 1, 1]
    ...
    def _calculate_dataset_statistics(self) -> Dict:
        ...
        return {
            ...
            'max_variance': float(np.max(all_variances)) if all_variances else 1000000.0,
            'mean_variance': float(np.mean(all_variances)) if all_variances else 500000.0
        }
    ...
    def calculate_image_ssc(self, image_data: Dict) -> Dict[str, float]:
        ...
        # 使用与recalculate_ssc_scores.py一致的公式
        ...
        # 使用全局最大方差作为sigma_a0
        sigma_a0 = self.dataset_stats['max_variance']
        ...
        ssc_score = float(np.dot(self.weights, scores))
