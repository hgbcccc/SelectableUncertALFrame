...
class UncertaintyMetrics:
    def __init__(self, ssc_weights: Optional[List[float]] = None):
        ...
        # 保存SSC权重参数
        self.ssc_weights = ssc_weights
    
    def compute_uncertainty(self, 
                        result: DetDataSample,
                        methods: Union[str, List[str]] = 'all',
                        min_score_thresh: float = 0.001,
            ...
            if method in self.metrics_mapping:
                try:
                    if method == 'box':
                        ...
                        # 跳过SSC计算，因为会在后面重新计算  # 解除注释可以计算ssc
                        # method_metrics = self.metrics_mapping[method](result, weights=self.ssc_weights)
                        method_metrics = {}  # 返回空字典，跳过SSC计算
                    else:
    ...
    def analyze_image_uncertainty(self, 
                                ...
                                min_score_thresh: float = 0.001) -> Dict[str, Dict[str, float]]:
        """分析单张图片的不确定性
...
class MUSCDBSampling(UncertaintyMetrics):
    """两阶段主动学习采样策略：MUS + CDB"""
    def __init__(self, class_names: Optional[List[str]] = None, ssc_weights: Optional[List[float]] = None):
        super().__init__(ssc_weights=ssc_weights)
