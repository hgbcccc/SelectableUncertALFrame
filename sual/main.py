...
class ModelHandler:
    ...
    def _load_previous_checkpoint(self, round_num: int, main_work_dir: Path):
        # 从主工作目录中查找上一轮的目录
        prev_iter_dir = main_work_dir / f"round_{round_num - 1}"
    ...
    def train_model(self, iter_work_dir: Path, round_num: int) -> Tuple[Runner, Dict]:
        ...
        if round_num > 1:
            # 传递主工作目录而不是当前轮次目录
            main_work_dir = iter_work_dir.parent
            self._load_previous_checkpoint(round_num, main_work_dir)
...
class UncertaintyHandler:
    ...
    def _infer_unlabeled_data(self, iter_work_dir: Path) -> Dict:
        ...
        # 修复路径问题：直接使用当前轮次的工作目录
        teacher_output_dir = iter_work_dir / 'teacher_outputs'
        ...
        if ('mus' in uncertainty_methods and 
            self.al_cfg.sample_selection.get('uncertainty_metric') == 'mus'):
            # 使用两阶段MUS-CDB采样策略
            self.logger.info("使用MUS-CDB两阶段采样策略")
...
class SampleSelector:
    ...
    def _create_selector(self, selector_type: str, train_stats: Dict):
        if selector_type == 'default':
            ...
            # 检查是否使用MUS-CDB策略
            if ('mus' in self.al_cfg.inference_options.get('uncertainty_methods', []) and 
                self.al_cfg.sample_selection.get('uncertainty_metric') == 'mus'):
                self.logger.info("配置为MUS-CDB策略")
    ...
    def _run_selector(self, selector, dataset, unlabeled_results, train_stats) -> List[str]:
        ...
        if selector_type == 'default':
            if isinstance(selector, MUSCDBSampling):
                # MUS-CDB两阶段采样策略
                self.logger.info("执行MUS-CDB两阶段采样")
                # 获取当前数据集的类别统计信息
                ...
                # 使用MUS-CDB的rank_samples_with_cdb方法
                selected_samples = selector.rank_samples_with_cdb(
                    results=results,
                    labeled_counts=category_stats,
                    img_names=img_names,
                    total_budget=self.al_cfg.sample_selection.get('sampling_budget', 3200),
                    theta=self.al_cfg.inference_options.get('score_thr', 0.1)
                )
            else:
                # 传统单阶段策略
        ...
        if selector_type == 'Wasserstein':
            ...
            q_table_path = str(self.cfg.work_dir / 'rl_q_table.json')
            return selector.select_samples(
                unlabeled_stats,
                select_num=self.al_cfg.sample_selection['num_samples'],
...
class ActiveLearningTrainer:
    ...
    def train(self):
        ...
        for self.round in range(self.round, self.max_iterations + 1):
            ...
            # 同步轮次信息到不确定性处理器
            uncertainty_handler.round = self.round
