...
from sual.utils.recalculate_ssc_scores import recalculate_ssc_scores
...
def calculate_train_stats(iter_work_dir: Path, al_cfg: Config, logger: MMLogger) -> Dict:
    ...
    # 从配置中获取SSC权重参数
    ssc_weights = al_cfg.get('ssc_weights', [2, 1, 2, 1, 1])
    
    calculator = SSCCalculator(data_cfg=al_cfg.train_pool_cfg, weights=ssc_weights)
...
def infer_unlabeled_data(cfg: Config, work_dir: Path, round_num: int, iter_work_dir: Path, logger: MMLogger) -> Dict:
    ...
    if ('mus' in uncertainty_methods and 
        ...
        uncertainty_calculator = MUSCDBSampling(ssc_weights=ssc_weights)
        use_two_stage = True
    else:
        # 使用传统的不确定性度量
        logger.info("使用单阶段不确定性度量")
        uncertainty_calculator = UncertaintyMetrics(ssc_weights=ssc_weights)
        use_two_stage = False
    ...
    # 重新计算SSC指标（树冠控制系数和边界框面积变异系数）
    logger.info("开始重新计算SSC指标...")
    results = recalculate_ssc_scores(results, results)
    logger.info("SSC指标重新计算完成")
    ...
    # 创建原始推理结果存储目录
    raw_dir = teacher_output_dir / 'raw_infer_results'
    raw_dir.mkdir(parents=True, exist_ok=True)
    
    logger.info("保存推理结果（最小内存占用方案）...")
    
    # 导入pickle用于序列化DetDataSample对象
    import pickle
    ...
    for img_name, info in results.items():
        if 'uncertainty' in info:
            # 1. 保存DetDataSample对象为二进制文件
            det_sample = info['result']
            raw_filename = img_name.replace('.jpg', '.pkl').replace('.png', '.pkl').replace('.jpeg', '.pkl')
            raw_path = raw_dir / raw_filename
            
            try:
                with open(raw_path, 'wb') as f:
                    pickle.dump(det_sample, f, protocol=pickle.HIGHEST_PROTOCOL)
                
                # 2. 只保留路径、可视化路径和不确定性信息
                minimal_results[img_name] = {
                    'raw_result_path': str(raw_path),
                    'vis_path': info.get('vis_path', ''),
                    'uncertainty': info['uncertainty']
                }
                
            except Exception as e:
                logger.error(f"保存DetDataSample对象失败 ({img_name}): {str(e)}")
                # 如果保存失败，仍然保留不确定性信息
                minimal_results[img_name] = {
                    'raw_result_path': '',
                    'vis_path': info.get('vis_path', ''),
    ...
    logger.info(f"推理结果已保存至: {uncertainty_path}")
    logger.info(f"原始推理对象已保存至: {raw_dir}")
    logger.info(f"主JSON文件大小: {uncertainty_path.stat().st_size / 1024:.2f} KB")
    logger.info(f"原始推理对象目录大小: {sum(f.stat().st_size for f in raw_dir.rglob('*') if f.is_file()) / 1024 / 1024:.2f} MB")
...
def create_selector(selector_type: str, train_stats: Dict, al_cfg: Config, logger: MMLogger):
    ...
    if selector_type == 'default':
        ...
        if ('mus' in al_cfg.inference_options.get('uncertainty_methods', []) and 
            ...
            return MUSCDBSampling(ssc_weights=ssc_weights)  # 使用MUS-CDB选择器
        else:
...
def load_det_sample_from_path(raw_result_path: str):
    """从路径加载DetDataSample对象"""
    import pickle
    from pathlib import Path
    
    if not raw_result_path or not Path(raw_result_path).exists():
        return None
    
    try:
        with open(raw_result_path, 'rb') as f:
            det_sample = pickle.load(f)
        return det_sample
    except Exception as e:
        print(f"加载DetDataSample对象失败 ({raw_result_path}): {str(e)}")
        return None


def load_inference_results_with_det_samples(uncertainty_results_path: str, img_names: List[str] = None):
    """加载推理结果并包含DetDataSample对象"""
    import pickle
    from pathlib import Path
    
    # 加载主JSON文件
    with open(uncertainty_results_path, 'r', encoding='utf-8') as f:
        results = json.load(f)
    
    # 如果指定了图片名称，只加载指定的图片
    if img_names:
        results = {name: results[name] for name in img_names if name in results}
    
    # 加载DetDataSample对象
    loaded_results = {}
    for img_name, info in results.items():
        raw_path = info.get('raw_result_path', '')
        det_sample = load_det_sample_from_path(raw_path)
        
        loaded_results[img_name] = {
            'result': det_sample,
            'vis_path': info.get('vis_path', ''),
