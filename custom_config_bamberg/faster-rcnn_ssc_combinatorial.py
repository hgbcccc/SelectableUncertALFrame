...
max_boxes_per_img = 80
nms_iou_threshold = 0.4


active_learning = dict(
    data_root='data/Bamberg_coco1024/active_learning_combinatorial',
    ann_file='data/Bamberg_coco1024/active_learning_combinatorial/annotations/instances_unlabeled.json',
    data_prefix=dict(img='data/Bamberg_coco1024/active_learning_combinatorial/images_unlabeled'),
    # 训练池的SSC计算配置   
    train_pool_cfg = dict(
        data_root='data/Bamberg_coco1024/active_learning_combinatorial',
...
#  这是在推理阶段使用的
# 决定要计算哪些不确定性指标
# 可选值包括：'basic', 'entropy', 'variance', 'ssc' 等
# 会影响 compute_uncertainty 方法的计算结果 是大方法
    max_iterations=16,

    sample_selection=dict(
        num_samples=565,  # 确保每轮选择200个样本
        uncertainty_metric='ssc_score',
        sample_selector="Combinatorial",  # 使用组合优化选择器
...
max_epochs=2  # 每轮主动学习内的训练轮数增加到3轮
...
test_evaluator = dict(
    ann_file='data/Bamberg_coco1024/annotations/instances_val2024.json',
    backend_args=None,
    format_only=False,
    metric='bbox',
    type='CocoMetric')
...
train_cfg = dict(max_epochs=2, type='EpochBasedTrainLoop', val_interval=1)  # 增加到3轮
train_dataloader = dict(
    batch_sampler=dict(type='AspectRatioBatchSampler'),
    batch_size=8,
    dataset=dict(
    ann_file='data/Bamberg_coco1024/active_learning_combinatorial/annotations/instances_labeled_train.json',
        backend_args=None,
        data_prefix=dict(img='data/Bamberg_coco1024/active_learning_combinatorial/images_labeled_train'),
        data_root='data/Bamberg_coco1024/active_learning_combinatorial',
...
train_pipeline = [
    ...
    dict(prob=0.5, type='RandomFlip'),
    dict(prob=0.5, direction='horizontal', type='RandomFlip'),  # 添加水平翻转
    dict(prob=0.3, direction='vertical', type='RandomFlip'),    # 添加垂直翻转
    # 增强光照变化，树冠检测对光影很敏感
    dict(
        brightness_delta=32,
        contrast_range=(0.7, 1.3),  # 调整对比度范围
        hue_delta=10,                # 减小色调变化
        saturation_range=(0.8, 1.2), # 减小饱和度变化
        type='PhotoMetricDistortion'
    ),
    # 增加马赛克增强，非常适合小目标检测
    dict(
...
val_dataloader = dict(
    ...
    dataset=dict(
        ann_file='data/Bamberg_coco1024/active_learning_combinatorial/annotations/instances_labeled_val.json',
        backend_args=None,
        data_prefix=dict(img='data/Bamberg_coco1024/active_learning_combinatorial/images_labeled_val'),
        data_root='data/Bamberg_coco1024/active_learning_combinatorial',
        metainfo=metainfo,
        pipeline=[
            dict(backend_args=None, type='LoadImageFromFile'),
...
work_dir = 'work_dirs/faster-rcnn_ssc_bamberg_combinatorial_16_200'  # 修改工作目录