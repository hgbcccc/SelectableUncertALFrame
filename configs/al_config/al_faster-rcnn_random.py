...
active_learning = dict(
    data_root='data/ForestDamages/active_learning_random',
    ann_file='data/ForestDamages/active_learning_random/annotations/instances_unlabeled.json',
    data_prefix=dict(img='data/ForestDamages/active_learning_random/images_unlabeled'),
    # 训练池的SSC计算配置   
    train_pool_cfg = dict(
        data_root='data/ForestDamages/active_learning_random',
    ...
    inference_options=dict(
        save_results=True,
        score_thr=0.01,
        uncertainty_methods=['basic'],  # 主方法
        selected_metric = 'sum_uncertainty',
        sample_size=10
    ),
    max_iterations=2,
    sample_selection=dict(
        num_samples=2,
        uncertainty_metric='random',  # 子方法
...
model = dict(
    ...
    test_cfg=dict(
        rcnn=dict(
            max_per_img=300,  # 增加以获取更多候选框
            nms=dict(iou_threshold=0.5, type='nms'),  # 放宽NMS阈值
            score_thr=0.005),  # 降低得分阈值
        rpn=dict(
            max_per_img=2000,
            min_bbox_size=0,
...
train_dataloader = dict(
    ...
    batch_size=4,
    dataset=dict(
    ann_file='data/ForestDamages/active_learning_random/annotations/instances_labeled_train.json',
        backend_args=None,
        data_prefix=dict(img='data/ForestDamages/active_learning_random/images_labeled_train'),
        data_root='data/ForestDamages/active_learning_random',
...
val_dataloader = dict(
    batch_size=4,
    dataset=dict(
        ann_file='data/ForestDamages/active_learning_random/annotations/instances_labeled_val.json',
        backend_args=None,
        data_prefix=dict(img='data/ForestDamages/active_learning_random/images_labeled_val'),
        data_root='data/ForestDamages/active_learning_random',
        metainfo=metainfo,
        pipeline=[
            dict(backend_args=None, type='LoadImageFromFile'),
