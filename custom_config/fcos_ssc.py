...
active_learning = dict(
    ...
    ann_file='data/ForestDamages/active_learning_cascade_ssc/annotations/instances_unlabeled.json',
    data_prefix=dict(img='data/ForestDamages/active_learning_cascade_ssc/images_unlabeled'),
    ...
    inference_options=dict(
        ...
        selected_metric='ssc_score',  # 使用SSC度量
        uncertainty_methods=['ssc']  # 使用SSC不确定性方法
    ),
    
    # 训练配置
    max_iterations=16,  # 主动学习轮次
    
    # 样本选择配置
    sample_selection=dict(
        num_samples=200,  # 每轮选择的样本数量
        sample_selector='default',  # 使用默认选择器
        uncertainty_metric='ssc_score',  # 使用SSC不确定性度量
        rl_metric='',
    ),
...
train_dataloader = dict(
    ...
    batch_size=2,
    dataset=dict(
        ann_file='data/ForestDamages/active_learning_cascade_ssc/annotations/instances_labeled_train.json',
        backend_args=None,
        data_prefix=dict(img='data/ForestDamages/active_learning_cascade_ssc/images_labeled_train'),
        data_root='data/ForestDamages/active_learning_cascade_ssc',
...
train_pipeline = [
    ...
    dict(angle_range=(-20, 20), type='RandomRotate'),
    dict(prob=0.5, type='RandomFlip'),
    dict(type='PackDetInputs'),
]
...
val_dataloader = dict(
    batch_size=4,
    dataset=dict(
        ann_file='data/ForestDamages/active_learning_cascade_ssc/annotations/instances_labeled_val.json',
        backend_args=None,
        data_prefix=dict(img='data/ForestDamages/active_learning_cascade_ssc/images_labeled_val'),
        data_root='data/ForestDamages/active_learning_cascade_ssc',
        metainfo=metainfo,
        pipeline=[
            dict(backend_args=None, type='LoadImageFromFile'),
...
# 工作目录配置
work_dir = 'work_dirs/fcos_ssc_16_200'
