            ...
            if metric == 'proposal':
                ...
                if self.classwise:  # Compute per-category AP
                    ...
                    for idx, cat_id in enumerate(self.cat_ids):
                        ...
                        # 修改这部分，添加 mAP_95 的计算
                        for iou in [0, 5, -1]:  # 添加 -1 表示最后一个 IoU 阈值 (0.95)
                    ...
                    # 修改 headers 添加 mAP_95
                    headers = [
                        'category', 'mAP', 'mAP_50', 'mAP_75', 'mAP_95', 'mAP_s',
                        'mAP_m', 'mAP_l'
                    ]
                ...
                if metric_items is None:
                    # 修改默认的 metric_items，添加 mAP_95
                    metric_items = [
                        'mAP', 'mAP_50', 'mAP_75', 'mAP_95', 'mAP_s', 'mAP_m', 'mAP_l'
                    ]

                for metric_item in metric_items:
                    key = f'{metric}_{metric_item}'
                    if metric_item == 'mAP_95':
                        # 特别处理 mAP_95
                        precisions = coco_eval.eval['precision']
                        precision_95 = precisions[-1, :, :, 0, -1]
                        val = np.mean(precision_95[precision_95 > -1])
                    else:
                ...
                # 获取刚计算的 mAP_95
                mAP_95 = eval_results[f'{metric}_mAP_95']
                # 修改输出格式，添加 mAP_95
                logger.info(f'{metric}_mAP_copypaste: {ap[0]:.3f} '
                            f'{ap[1]:.3f} {ap[2]:.3f} {mAP_95:.3f} '
