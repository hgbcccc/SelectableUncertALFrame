...
class BBoxHead(BaseModule):
    ...
    def _predict_by_feat_single(
        ...
        results = InstanceData()
        if roi.shape[0] == 0:
            empty_results = empty_instances([img_meta],
                                   roi.device,
                                   task_type='bbox',
                                   instance_results=[results],
                                   box_type=self.predict_box_type,
                                   ...
                                   num_classes=self.num_classes,
                                   score_per_cls=rcnn_test_cfg is None)[0]
            # Add empty all_scores to match the structure
            empty_results.all_scores = torch.empty((0, self.num_classes), device=roi.device)
        ...
        if rcnn_test_cfg is None:
            ...
            results.bboxes = bboxes
            results.scores = scores
            # Store all class scores (excluding background)
            results.all_scores = scores[:, :-1] if scores is not None else None
        else:
            # Standard NMS path
            raw_scores_with_bg = scores.clone() # (num_rois, num_classes + 1)
            num_classes_no_bg = self.num_classes # This should be raw_scores_with_bg.size(1) - 1 if num_classes can vary
            if raw_scores_with_bg.size(1) != (self.num_classes + 1):
                 # Add a check or warning if this assumption is violated.
                 # For robustness, use actual dim: num_classes_no_bg = raw_scores_with_bg.size(1) -1
                 pass # Placeholder for actual warning/error logic

            det_bboxes_with_max_score, det_labels, final_flat_indices = multiclass_nms(
                bboxes, 
                raw_scores_with_bg, 
                rcnn_test_cfg.score_thr,
                rcnn_test_cfg.nms,
            ...
            results.bboxes = det_bboxes_with_max_score[:, :-1]
            results.scores = det_bboxes_with_max_score[:, -1]
            results.labels = det_labels

            if final_flat_indices.numel() > 0:
                # final_flat_indices are indices into the (num_rois * num_classes_no_bg) flat list
                # (potentially after score thresholding inside multiclass_nms for non-ONNX path)
                # We need to map these back to the original RoI indices.
                # Use the actual number of classes from the scores tensor for division
                actual_num_classes_for_division = raw_scores_with_bg.size(1) -1
                if actual_num_classes_for_division <= 0: # Avoid division by zero or negative if scores tensor is malformed
                    # Handle error case: perhaps log a warning and set empty all_scores
                    results.all_scores = torch.empty((0, self.num_classes), device=raw_scores_with_bg.device)
                else:
                    original_roi_indices = final_flat_indices // actual_num_classes_for_division
                    
                    # Extract all class scores (excluding background) for the kept detections
                    all_scores_for_kept_dets = raw_scores_with_bg[original_roi_indices, :-1]
                    results.all_scores = all_scores_for_kept_dets
            else:
                # If no detections after NMS, or final_flat_indices is empty for other reasons.
                # Ensure all_scores is correctly sized even if empty.
                num_classes_to_use_for_empty = self.num_classes
                if raw_scores_with_bg is not None and raw_scores_with_bg.numel() > 0:
                     num_classes_to_use_for_empty = raw_scores_with_bg.size(1) -1
                     if num_classes_to_use_for_empty < 0: num_classes_to_use_for_empty = 0 # handle malformed scores
                results.all_scores = torch.empty((0, num_classes_to_use_for_empty), device=raw_scores_with_bg.device if raw_scores_with_bg is not None else roi.device)
        return results
