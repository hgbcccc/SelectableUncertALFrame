...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def _predict_by_feat_single(self,
        ...
        mlvl_scores = []
        mlvl_labels = []
        mlvl_all_scores = []  # 新增: 存储每个level的所有类别概率
        if with_score_factors:
            mlvl_score_factors = []
        else:
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            # 新增: 保存过滤后对应的原始概率分布
            mlvl_all_scores.append(original_scores[keep_idxs])
        ...
        # 新增: 添加all_scores字段
        results.all_scores = torch.cat(mlvl_all_scores)
        if with_score_factors:
    ...
    def _bbox_post_process(self,
        ...
        if with_nms and results.bboxes.numel() > 0:
            ...
            # 确保在NMS后保留all_scores
            all_scores = results.all_scores[keep_idxs] if hasattr(results, 'all_scores') else None
